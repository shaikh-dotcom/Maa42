...
import { Heart, Zap, Droplet, CheckCircle, Play, Pause, ChevronRight, Activity } from 'lucide-react';
...
export const TrackerScreen: React.FC<TrackerScreenProps> = ({ user, initialTrackId, onClearInitialTrackId }) => {
  const [energyLevel, setEnergyLevel] = useState(6);
  const [lochiaLevel, setLochiaLevel] = useState(2);
  ...
  const getLochiaLabel = (val: number) => {
    const labels = ['None', 'Light Spotting', 'Moderate Flow', 'Consult Doctor'];
    return labels[val - 1] || 'Light Spotting';
  };
  ...
  return (
    ...
    >
      ...
      <section className="relative overflow-hidden rounded-3xl bg-primary-container text-on-primary-container p-6 shadow-md mb-6 border border-primary/20">
        ...
        <div className="relative z-10 flex flex-col gap-2">
          <div className="inline-flex items-center gap-2 bg-on-primary-container/20 px-3 py-1 rounded-full text-xs font-semibold w-fit">
            ...
            <span>Maa42 Recovery Program</span>
          </div>
          <h1 className="text-2xl font-bold font-headline text-on-primary">
            Fourth Trimester &bull; Day 28 of 42
          </h1>
          <p className="text-sm text-on-primary-container/95 max-w-md leading-relaxed">
            You are 66% through the sacred postpartum window. Rest, nourishment, and gentle core rehabilitation are your focus today.
          </p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        {/* Recovery Window Progress Ring Card */}
        <motion.div
          whileHover={{ y: -3 }}
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-semibold text-on-surface">42-Day Recovery Window</h2>
          ...
          <div className="flex items-center justify-center py-4">
            <div className="relative flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center text-center">
                <span className="text-3xl font-extrabold text-on-surface">Day 28</span>
                <span className="text-xs text-on-surface-variant font-medium">of 42</span>
              </div>
            </div>
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-on-surface">Daily Check-In</h2>
          ...
          <div className="flex flex-col gap-4 my-auto">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-bold text-primary">{getEnergyLabel(energyLevel)}</span>
              </div>
              <input
                type="range"
                min="1"
            ...
            {/* Lochia / Bleeding Status Slider */}
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-medium text-on-surface-variant flex items-center gap-1">
                  <Droplet className="w-3.5 h-3.5 text-secondary fill-current" />
                  <span>Lochia / Bleeding</span>
                </span>
                <span className="font-bold text-secondary">{getLochiaLabel(lochiaLevel)}</span>
              </div>
              <input
                type="range"
                min="1"
                max="4"
                value={lochiaLevel}
                onChange={(e) => setLochiaLevel(Number(e.target.value))}
      ...
      <AnimatePresence>
        {logSavedToast && (
          ...
          >
            ...
            <span>Daily recovery log updated &amp; synchronized with Dr. Sharma&apos;s chart.</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-on-surface uppercase tracking-wider">
            Curated Recovery Care &amp; Audio Therapy
          </h2>
          <span className="text-xs font-semibold text-primary">Web Audio Synthesizer</span>
        </div>
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-105 ${
                    isPlayingThis ? 'bg-primary text-on-primary animate-pulse' : track.colorClass
                  }`}
                >
                  {isPlayingThis ? (
