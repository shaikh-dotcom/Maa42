...
import { Bot, ShieldCheck, ListChecks, ArrowUp, Plus, AlertCircle, Sparkles, HeartHandshake } from 'lucide-react';
...
const DEFAULT_MESSAGES: ChatMessage[] = [
  {
    id: 'msg-1',
    sender: 'user',
    text: "Hi Sophia, I've been feeling some mild pelvic pressure today. Is this normal around week 24?",
    timestamp: 'Today, 9:42 AM',
  },
  {
    id: 'msg-2',
    sender: 'sophia',
    text: "Hello! It is completely understandable to check on this. Mild pelvic pressure around Week 24 is often caused by the growing uterus and shifting ligaments (like round ligaments). However, let's make sure you're safe and comfortable.",
...
export const SophiaScreen: React.FC<SophiaScreenProps> = ({
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const replyText = data.reply || "I am here to support you. Let's make sure you're feeling rested and cared for today.";

      const isUrgent = textToSend.toLowerCase().includes('bleed') || textToSend.toLowerCase().includes('severe') || textToSend.toLowerCase().includes('leak');
      ...
      const botMsg: ChatMessage = {
        ...
        triage: isUrgent
          ? {
              ...
              description: 'Please reach out to your lead OB-GYN or use SOS hotline.',
              level: 'high',
            }
          : undefined,
      };
      ...
      const fallbackMsg: ChatMessage = {
        id: `sophia-${Date.now()}`,
        sender: 'sophia',
        text: `Thank you for sharing. Around Week ${user.week}, your body is doing tremendous work growing baby to roughly 600g. If you feel tightness, take a gentle 15-minute rest on your left side with an electrolyte beverage. Dr. Sharma's team is alerted if any persistent discomfort arises.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
    ...
    >
      ...
      <div className="bg-primary-container text-on-primary-container p-6 rounded-3xl shadow-md relative overflow-hidden mb-6 border border-primary/20">
        ...
        <div className="flex items-start justify-between relative z-10 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-bold text-on-primary">Sophia AI Assistant</h2>
            </div>
          </div>
        ...
        <p className="relative z-10 text-sm text-on-primary-container/95 leading-relaxed">
          Your personalized companion for Week {user.week}. I am here to answer clinical questions, provide symptom relief guides, and coordinate with your care team.
        </p>
      </div>
                        ...
                        <div>
                          <span
                            className={`text-xs font-bold block ${
                              msg.triage.level === 'high' ? 'text-error' : 'text-primary'
                            }`}
                          >
                            {msg.triage.status}
                          </span>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          ...
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.96 }}
            onClick={() => handlePromptClick('Week 24 milestone checklist')}
            className="bg-surface-container hover:bg-surface-container-high text-on-surface px-4 py-2 rounded-full text-xs font-medium shrink-0 transition-colors shadow-xs flex items-center gap-1.5 cursor-pointer"
          >
            <Sparkles className="w-3.5 h-3.5 text-primary" />
            <span>Week 24 milestone checklist</span>
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.96 }}
      ...
      <div className="sticky bottom-20 z-30 bg-surface/90 backdrop-blur-md pt-2 pb-2">
        ...
        >
          <button
            type="button"
            onClick={() => handlePromptClick("Can you give me a summary of red flag symptoms I should watch for today?")}
        ...
        <div className="text-center mt-2.5">
          <span className="text-[11px] text-on-surface-variant">
            Sophia provides guidance, not formal medical diagnosis. Contact your OB/GYN for emergencies.
          </span>
        </div>
      </div>
    </motion.div>
