...
import { X, UserPlus, MessageSquare, Heart, Check } from "lucide-react";
...
export const NotificationsPanel: React.FC<NotificationsPanelProps> = ({
  ...
  const requestFor = (n: AppNotification) =>
    incomingRequests.find((r) => r.id === n.requestId);
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-surface-container">
          ...
          {!isLoading &&
            ...
            const pendingRequest =
              n.type === "friend_request" ? requestFor(n) : undefined;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {n.type === "friend_request" && pendingRequest && (
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          acceptFriendRequest(pendingRequest);
                          clearNotification(n.id);
                        }}
                        className="flex items-center gap-1 text-[11px] font-semibold bg-primary text-on-primary px-3 py-1.5 rounded-full cursor-pointer"
                      >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          declineFriendRequest(pendingRequest.id);
                          clearNotification(n.id);
                        }}
                        className="text-[11px] font-semibold bg-surface-container-high text-on-surface-variant px-3 py-1.5 rounded-full cursor-pointer"
                      >
                        Decline
                      </button>
                    </div>
